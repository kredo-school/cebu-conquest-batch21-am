...
export default class MainScene extends Phaser.Scene {
  ...
  _onMapClicked(x, y) {
    ...
    if (this.isSelectionMode) {
      ...
      if (!isNeighbor && id !== this.currentDistrictId) {
        this.showLog("⚠️ 遠すぎます！隣接する地区しか攻撃できません。");
        return;
      }

      console.log(`⚔️ 地区 ${id} への作戦モーダルを表示`);
      store.setModal(true, { id: id, name: this.districts[id].name });
    }
  }
  ...
  _loadDistrictsFromTMJ() {
    ...
    objectLayer.objects.forEach((obj) => {
      ...
      this.districts[id] = {
        id, name: obj.name, polygon: poly, center: this._calcCenter(poly),
        owner: "neutral", graphics: null, textLabel: null,
      };
    });
  }
