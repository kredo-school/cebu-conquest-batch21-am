...
export const HelpModal: React.FC<HelpModalProps> = ({ onClose }) => {
  return (
    <div className="fixed inset-0 z-[100000] flex items-center justify-center p-4 md:p-8 animate-fadeIn text-left">
      {/* Background Blur */}
      <div className="absolute inset-0 bg-slate-950/80 backdrop-blur-md" onClick={onClose}></div>

      {/* Monitor Panel */}
      <div className="relative w-full max-w-4xl bg-slate-900 border border-cyan-500/30 rounded-2xl shadow-[0_0_50px_rgba(6,182,212,0.2)] overflow-hidden flex flex-col max-h-[90vh] text-left">
        
        {/* Header */}
        <div className="p-6 border-b border-cyan-500/20 bg-cyan-500/5 flex justify-between items-center text-left">
          <div>
            <h2 className="text-2xl font-black text-cyan-400 italic tracking-tighter uppercase font-fix">Tactical Manual</h2>
            {/* 🚀 修正: mt-1 から mt-2 に変更して間隔を空け、コロンの前後にスペースを入れた */}
            <p className="text-[10px] text-cyan-700 font-bold tracking-[0.3em] uppercase mt-2 font-fix">
              Operation : Cebu Conquest
            </p>
          </div>
        ...
        {/* Content Area */}
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-12 text-left">
          ...
          <section>
            ...
            <p className="text-slate-400 text-sm leading-relaxed pl-11 font-fix text-left">
          ...
          <section>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pl-11 text-left">
              <div className="bg-slate-800/50 p-4 rounded-xl border border-white/5">
                <span className="text-cyan-400 font-black text-[10px] uppercase block mb-2 font-fix text-left">AP (Action Points)</span>
                <p className="text-slate-300 text-xs font-fix text-left">Required for execution parameters including relocation, offensive assaults, and sector annexation. Regenerates automatically at the start of each turn matrix.</p>
              </div>
              <div className="bg-slate-800/50 p-4 rounded-xl border border-white/5">
                <span className="text-orange-400 font-black text-[10px] uppercase block mb-2 font-fix text-left">Armor (HP)</span>
                <p className="text-slate-300 text-xs font-fix text-left">Represents baseline unit structural durability. Dropping to 0 results in complete combat link disconnection, risking total network grid forfeiture.</p>
              </div>
            </div>
          </section>
          ...
          <section>
            ...
            <p className="text-slate-400 text-sm leading-relaxed pl-11 mb-4 font-fix text-left">
            ...
            <ul className="grid grid-cols-2 gap-3 pl-11 text-[11px] text-slate-500 font-bold uppercase italic text-left">
