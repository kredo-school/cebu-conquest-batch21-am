...
export const GlobalNavbar: React.FC<Props> = ({ 
  ...
  const { playerName, myId, playerAvatar, myTeam, logout, isAuthenticated, view } = useGameStore();
  const [isHovered, setIsHovered] = useState(false);

  // 現在のview（画面）が ranking や setup ならボタンを非表示にする
  const isRankingView = view === 'ranking';
  // 🚀 修正：存在しない 'settings' の判定を削除し、型エラーを解消
  const isSettingsView = view === 'setup';
  ...
  return (
    <nav className="fixed top-0 w-full z-50 flex justify-between items-center px-8 py-4 bg-slate-950/80 backdrop-blur-md border-b border-white/5 shadow-md">
      ...
      <div className="flex gap-4 items-center">
        {/* ランキング画面が開いている時はボタンを隠す */}
        {onOpenRanking && !isRankingView && (
        ...
        {/* 設定画面(setup)が開いている時はボタンを隠す */}
        {onOpenSettings && !isSettingsView && (
        ...
        {/* ログイン済み（isAuthenticated === true）の時だけプロファイルを表示する */}
        {isAuthenticated ? (
          <div 
          ...
          >
            ...
            {/* ミニモーダルの当たり判定抜け（隙間落ち）を防止 */}
            {isHovered && (
              <div className="absolute top-full right-0 pt-3 w-60 z-50">
